...
export function Navigation({ items }: NavigationProps) {
  ...
  return (
    <div className="flex justify-center p-1 space-x-6 py-3 w-full bg-grey-850">
        ...
        >
          <span
            className={classNames("px-4 py-1 rounded-full", {
              "bg-grey-800": item.isActiveRegex
          ...
          <span className="text-[13px] text-grey-200">{item.label}</span>
        </Link>
      ))}
    </div>
  );

  ...
  const user = await prisma.user.create({
    data: {
      ...
      ...(userData.wallets
        ...
        : { wallets: { create: { address: `${userId}_wallet` } } }),
    },
  });
...
async function createStack(
  stackData: Partial<Prisma.StackCreateInput> = {},
): Promise<Stack> {
...
async function createQuestion(
  questionData: Omit<Partial<Prisma.QuestionCreateInput>, "stack"> & {
    stackId?: number;
  },
): Promise<Question> {
...
async function createChompResult(chompResultData: {
  ...
  const dataToCreate: Prisma.ChompResultCreateInput = {
    ...
    rewardTokenAmount: chompResultData.rewardTokenAmount
      ? new Decimal(chompResultData.rewardTokenAmount)
      : new Decimal(0),
    result: chompResultData.result || ResultType.Claimed,
    transactionStatus:
      chompResultData.transactionStatus || TransactionStatus.Completed,
    ...(chompResultData.customData || {}),
  };

  // Add revealNft if result is Claimed or Revealed and no other identifier is present
  if (
    (dataToCreate.result === ResultType.Claimed ||
      dataToCreate.result === ResultType.Revealed) &&
    !(
      chompResultData.customData && "revealNft" in chompResultData.customData
    ) && // check if revealNft is already in customData
    !(
      chompResultData.customData &&
      "burnTransactionSignature" in chompResultData.customData
    ) && // check if burnTransactionSignature is in customData
...
async function cleanupDatabase() {
  ...
  await prisma.mysteryBoxPrize.deleteMany({
    where: { id: { in: createdMysteryBoxPrizeIds } },
  });
  await prisma.mysteryBoxTrigger.deleteMany({
    where: { id: { in: createdMysteryBoxTriggerIds } },
  });
  await prisma.mysteryBox.deleteMany({
    where: { id: { in: createdMysteryBoxIds } },
  });
  ...
  await prisma.chompResult.deleteMany({
    where: { id: { in: createdChompResultIds } },
  });
  ...
  await prisma.fungibleAssetTransactionLog.deleteMany({
    where: {
      OR: [
        { questionId: { in: createdQuestionIds } },
...
describe("getTotalBonkClaimed", () => {
  ...
  async function createMysteryBox(
    mbData: Omit<Partial<Prisma.MysteryBoxCreateInput>, "user"> & {
      userId: string;
    },
  ): Promise<MysteryBox> {
  ...
  async function createMysteryBoxTrigger(
    mbtData: Omit<
      Partial<Prisma.MysteryBoxTriggerCreateInput>,
      "MysteryBox" | "question" | "deck"
    > & { mysteryBoxId: string; questionId?: number; deckId?: number },
  ...
  async function createMysteryBoxPrize(
    mbpData: Omit<
      Partial<Prisma.MysteryBoxPrizeCreateInput>,
      "mysteryBoxTrigger"
    ...
    const data: Prisma.MysteryBoxPrizeCreateInput = {
      ...
      ...(restMbpData.prizeType === "Token" && {
        tokenAddress: restMbpData.tokenAddress || BONK_TOKEN_ADDRESS,
      }),
      ...restMbpData,
    };
  ...
  test("should return correct BONK amount for a user with MysteryBoxPrizes only", async () => {
    ...
    const mbt = await createMysteryBoxTrigger({
      mysteryBoxId: mb.id,
      questionId: question1.id,
    });
    await createMysteryBoxPrize({
      mysteryBoxTriggerId: mbt.id,
      amount: "200",
  ...
  test("should rank multiple users correctly with mixed rewards", async () => {
    ...
    const mbt1 = await createMysteryBoxTrigger({
      mysteryBoxId: mb1.id,
      questionId: q2.id,
    });
    await createMysteryBoxPrize({
    ...
    const mbt3 = await createMysteryBoxTrigger({
      mysteryBoxId: mb3.id,
      questionId: q4.id,
    });
    await createMysteryBoxPrize({
  ...
  test("should filter by stackId correctly", async () => {
    ...
    await createChompResult({
      userId: user1.id,
      questionId: q_s1_1.id,
      rewardTokenAmount: 100,
    });
    const mb_u1_s1 = await createMysteryBox({
      userId: user1.id,
      id: "mb_u1_s1",
    });
    const mbt_u1_s1 = await createMysteryBoxTrigger({
      mysteryBoxId: mb_u1_s1.id,
      questionId: q_s1_1.id,
    });
    await createMysteryBoxPrize({
      mysteryBoxTriggerId: mbt_u1_s1.id,
      amount: "50",
      tokenAddress: BONK_TOKEN_ADDRESS,
    });
    await createChompResult({
      userId: user2.id,
      questionId: q_s2_1.id,
      rewardTokenAmount: 200,
    });
    const mb_u2_s2 = await createMysteryBox({
      userId: user2.id,
      id: "mb_u2_s2",
    });
    const mbt_u2_s2 = await createMysteryBoxTrigger({
      mysteryBoxId: mb_u2_s2.id,
      questionId: q_s2_1.id,
    });
    await createMysteryBoxPrize({
      mysteryBoxTriggerId: mbt_u2_s2.id,
      amount: "75",
      tokenAddress: BONK_TOKEN_ADDRESS,
    });
    await createChompResult({
    ...
    const resultStack1 = await getTotalBonkClaimed(
      {},
      stack1.id,
      createdUserIds,
    );
    ...
    const resultStack2 = await getTotalBonkClaimed(
      {},
      stack2.id,
      createdUserIds,
    );
    ...
    const user1Stack2Data = resultStack2.ranking.find(
      (r) => r.user.id === user1.id,
    );
  ...
  test("should filter by dateFilter correctly for claimedAt (MysteryBox) and createdAt (Chomp)", async () => {
    ...
    await createChompResult({
      ...
      questionId: question1.id,
      rewardTokenAmount: 100,
      customData: { createdAt: dateInFilter },
    });
    // Chomp result outside the date filter range (uses question2)
    await createChompResult({
      userId: user1.id,
      questionId: question2.id,
      rewardTokenAmount: 500,
      customData: { createdAt: dateBeforeFilter },
    });

    const mb_in = await createMysteryBox({
      userId: user1.id,
      id: "mb_in_date",
    });
    // Link mystery box to one of the questions, e.g., question1 for consistency, or a new one if needed
    const mbt_in = await createMysteryBoxTrigger({
      mysteryBoxId: mb_in.id,
      questionId: question1.id,
    });
    await createMysteryBoxPrize({
      mysteryBoxTriggerId: mbt_in.id,
      amount: "200",
      tokenAddress: BONK_TOKEN_ADDRESS,
      claimedAt: dateInFilter,
    });

    const mb_after = await createMysteryBox({
      userId: user1.id,
      id: "mb_after_date",
    });
    const mbt_after = await createMysteryBoxTrigger({
      mysteryBoxId: mb_after.id,
      questionId: question2.id,
    });
    await createMysteryBoxPrize({
      mysteryBoxTriggerId: mbt_after.id,
      amount: "700",
      tokenAddress: BONK_TOKEN_ADDRESS,
      claimedAt: dateAfterFilter,
    });
  ...
  test("should handle non-numeric or zero amounts from MysteryBoxPrizes gracefully", async () => {
    ...
    const mbt2 = await createMysteryBoxTrigger({
      mysteryBoxId: mb.id,
      questionId: q3.id,
      id: "mbt_zero",
    });
  ...
  test("should correctly show loggedInUserScore if a current user is present", async () => {
    const user1 = await createUser({
      id: "user1_logged_in",
      username: "User1LoggedIn",
    });
    const loggedInUser = await createUser({
      id: "user2_logged_in",
      username: "User2LoggedIn",
    });
